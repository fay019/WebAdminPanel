<?php /* PhpManage MVC view — preserves legacy markup and hooks */ ?>
<div class="card">
    <h2>PHP (Système)</h2>
    <?php show_flash(); ?>
        ...
        <div>
            ...
            <form method="post"
                ...
                <div>
                    ...
                    <input name="ver_custom" placeholder="ex: 7.4">
                </div>
                <button class="btn primary" name="action" value="install" data-install data-stream="1">Installer</button>
            </form>
        </div>
    </div>
        ...
        <?php else: foreach ($rows as $r): ?>
            <tr>
                ...
                <td class="col-actions" data-label="Actions">
                    <form method="post" action="/php/manage/action" class="action-form">
                        <?= csrf_input() ?>
                        <input type="hidden" name="ver" value="<?= htmlspecialchars($r['ver']) ?>">
                        <button class="btn" name="action" value="restart" data-stream="1">Redémarrer</button>
                    </form>
                    <form method="post" action="/php/manage/action" class="action-form">
                        <?= csrf_input() ?>
                        <input type="hidden" name="ver" value="<?= htmlspecialchars($r['ver']) ?>">
                        <button class="btn danger" name="action" value="remove" data-confirm="Désinstaller PHP <?= htmlspecialchars($r['ver']) ?> ?" data-stream="1">
                            Désinstaller
                        </button>
                    </form>
                </td>
